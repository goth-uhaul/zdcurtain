...
uv run --active pyside6-rcc './resources.qrc' -o './src/gen/resources_rc.py'
...
If (-not $GITHUB_REPOSITORY) {
  $GITHUB_REPOSITORY = 'goth-uhaul/ZDCurtain'
}

New-Item $build_vars_path -ItemType File -Force | Out-Null
Add-Content $build_vars_path "ZDCURTAIN_BUILD_NUMBER = `"$BUILD_NUMBER`""
Add-Content $build_vars_path "ZDCURTAIN_GITHUB_REPOSITORY = `"$GITHUB_REPOSITORY`""
