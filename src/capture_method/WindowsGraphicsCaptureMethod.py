...
Caps at around 60 FPS."""
    ...
    def __init__(self, zdcurtain: "ZDCurtain"):
        ...
        if not is_valid_hwnd(zdcurtain.hwnd):
            return
        ...
        item = create_for_window(zdcurtain.hwnd)
    ...
    def close(self):
        ...
        if self.session:
            try:
                ...
                # "ZDCurtain.exe	<process started at 00:05:37.020 has terminated with 0xc0000409 (EXCEPTION_STACK_BUFFER_OVERRUN)>" # noqa: E501
                pass
            self.session = None

    @override
    def get_frame(self) -> MatLike | None:
        selection = self._zdcurtain_ref.settings_dict["capture_region"]
    ...
    def check_selected_region_exists(self):
        return bool(
            is_valid_hwnd(self._zdcurtain_ref.hwnd)  # fmt: skip
            and self.frame_pool
            and self.session
        )
